...
function startServer(graph) {
  ...
  app.get('/graph/version/:pkg', function (req, res) {
    // TODO: remove duplcate checks
    var nodeId = req.params.pkg;
    if (!graph.getNode(nodeId)) {
      res.status(404).send('Package is not found');
    ...
    res.send(graph.getVersionUsage(nodeId, true /* include names */));
  });

  app.get('/graph/version/:pkg/stats?', function (req, res) {
    // TODO: remove duplcate checks
    var nodeId = req.params.pkg;
    if (!graph.getNode(nodeId)) {
      res.status(404).send('Package is not found');
    ...
    res.send(graph.getVersionUsage(nodeId, false /* do not names, print stats only */));
  });
